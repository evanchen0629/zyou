...
async function writeData(type, path, data) {
    ...
    const { lines } = await (0, quickType_1.quicktypeJSONSchema)('TypeScript', (0, exports.typeMapping)(config)[type].format(path.toLowerCase().replace(/( |^)[a-z]/g, (L) => L.toUpperCase())), JSON.stringify(data));
